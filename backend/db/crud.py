...
async def add_phase(db: AsyncSession, session_id: str, phase_index: int, name: str, description: str = "", files: list | None = None) -> Phase:
    phase = Phase(session_id=session_id, phase_index=phase_index, name=name, description=description, files=files)
    db.add(phase)
    await db.commit()
